...
import org.springframework.data.rest.repository.*;
...
/**
 * This mirrors {@code RepositoryRestController} with looser access specifiers and includes some major bugfixes.<br>
 * <p/>
 * Based on spring-data-rest 1.0.0.RELEASE
 *
 * @see RepositoryRestController
 */
public class FlexibleRepositoryRestController extends RepositoryRestController implements ApplicationContextAware, InitializingBean {

    public static final String LOCATION = "Location";
    public static final String SELF = "self";
    //public static final ThreadLocal<URI> BASE_URI = new ThreadLocal<URI>();

    protected static final Logger LOG = LoggerFactory.getLogger(RepositoryRestController.class);
    protected static final TypeDescriptor STRING_ARRAY_TYPE = TypeDescriptor.valueOf(String[].class);
     ...
     */
    protected DelegatingConversionService conversionService = new DelegatingConversionService(new DefaultFormattingConversionService());
    protected MethodParameterConversionService methodParameterConversionService = new MethodParameterConversionService(conversionService);
    /**
     ...
     */
    protected SortedSet<String> availableMediaTypes = new TreeSet<String>();
    protected RepositoryRestConfiguration config = RepositoryRestConfiguration.DEFAULT;
    protected ObjectMapper objectMapper = new ObjectMapper();
    protected RepositoryAwareMappingHttpMessageConverter mappingHttpMessageConverter;
    protected UriToDomainObjectUriResolver domainObjectResolver;
    ...
    public void setHttpMessageConverters(List<HttpMessageConverter> httpMessageConverters) {
        if (this.availableMediaTypes == null) {
            // Call from the superclass' initialization block
            return;
        }
    ...
    public void afterPropertiesSet() throws Exception {
        for (ConversionService cs : BeanFactoryUtils.beansOfTypeIncludingAncestors(applicationContext, ConversionService.class).values()) {
    ...
    public ResponseEntity<?> listRepositories(ServletServerHttpRequest request, URI baseUri) throws IOException {
        ...
        return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), new Resources<String>(Collections.<String>emptyList(), links));
    }
    ...
    public ResponseEntity<?> listEntities(ServletServerHttpRequest request, PagingAndSorting pageSort, URI baseUri, @PathVariable String repository) throws IOException {
        ...
        if (repoMeta.repository() instanceof PagingAndSortingRepository) {
            ...
            pageMeta = new PagedResources.PageMetadata(page.getSize(), page.getNumber() + 1, page.getTotalElements(), page.getTotalPages());
            ...
            maybeAddPrevNextLink(nextPrevBase, repoMeta, pageSort, page, !page.isFirstPage() && page.hasPreviousPage(), page.getNumber(), "prev", links);
            maybeAddPrevNextLink(nextPrevBase, repoMeta, pageSort, page, !page.isLastPage() && page.hasNextPage(), page.getNumber() + 2, "next", links);
        } else {
        ...
        if (!repoMeta.queryMethods().isEmpty()) {
            links.add(new Link(buildUri(baseUri, repository, "search").toString(), repoMeta.rel() + ".search"));
        }

        return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), (null != pageMeta ? new PagedResources(allResources, pageMeta, links) : new Resources(allResources, links)));
    }
    ...
    public ResponseEntity<?> listQueryMethods(ServletServerHttpRequest request, URI baseUri, @PathVariable String repository) throws IOException {
        ...
        for (Map.Entry<String, RepositoryQueryMethod> entry : ((Map<String, RepositoryQueryMethod>) repoMeta.queryMethods()).entrySet()) {
            ...
            if (m.isAnnotationPresent(RestResource.class)) {
                ...
                links.add(new Link(buildUri(baseSearchUri, (StringUtils.hasText(resourceAnno.path()) ? resourceAnno.path() : entry.getKey())).toString(), (StringUtils.hasText(resourceAnno.rel()) ? repoMeta.rel() + "." + resourceAnno.rel() : repoMeta.rel() + "." + entry.getKey())));
            } else {
                // No customizations, use the default
                links.add(new Link(buildUri(baseSearchUri, entry.getKey()).toString(), repoMeta.rel() + "." + entry.getKey()));
            }
        }
    ...
    public ResponseEntity<?> query(ServletServerHttpRequest request, PagingAndSorting pageSort, URI baseUri, @PathVariable String repository, @PathVariable String query) throws InvocationTargetException, IllegalAccessException, IOException {
        ...
        if (null == (result = queryMethod.method().invoke(repo, paramVals))) {
            return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), new Resources<String>(Collections.<String>emptyList()));
        }
        ...
        return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), (null != pageMetadata ? new PagedResources(results, pageMetadata, links) : new Resources(results, links)));
    }
    ...
    public ResponseEntity<?> entity(ServletServerHttpRequest request, URI baseUri, @PathVariable String repository, @PathVariable String id) throws IOException {
        ...
        return negotiateResponse(request, HttpStatus.OK, headers, entity);
    }
    ...
    public ResponseEntity<?> propertyOfEntity(ServletServerHttpRequest request, URI baseUri, @PathVariable String repository, @PathVariable String id, @PathVariable String property) throws IOException {
        ...
        if (propVal instanceof Collection) {
            ...
            return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), new Resources(outgoing, links));
            ...
            return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), new Resource(resource, links));
            ...
            if (shouldReturnLinks(accept)) {
                ...
                return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), new Resources(Collections.emptyList(), links));
                ...
                return negotiateResponse(request, HttpStatus.OK, new HttpHeaders(), r);
    ...
    public ResponseEntity<?> updatePropertyOfEntity(final ServletServerHttpRequest request, URI baseUri, @PathVariable String repository, @PathVariable String id, final @PathVariable String property) throws IOException {
        ...
        MediaType incomingMediaType = request.getHeaders().getContentType();
        Resource incomingLinks = readIncoming(request, incomingMediaType, Resource.class);
    ...
    @ExceptionHandler(
            {
                    InvocationTargetException.class, IllegalArgumentException.class, ClassCastException.class, ConversionFailedException.class
            }
    )
    @ResponseBody
    ...
    public ResponseEntity handleJsr303ValidationFailure(ConstraintViolationException ex, ServletServerHttpRequest request) throws IOException {
        ...
        for (ConstraintViolation cv : ex.getConstraintViolations()) {
            String msg = applicationContext.getMessage(cv.getMessageTemplate(), new Object[]{
                    cv.getLeafBean().getClass().getSimpleName(), cv.getPropertyPath().toString(), cv.getInvalidValue()
            }, cv.getMessage(), null);
            errors.add(msg);
        }
        m.put("errors", errors);
   ...
   */
    ...
    @SuppressWarnings({"unchecked"})
    protected void maybeAddPrevNextLink(URI resourceUri, RepositoryMetadata repoMeta, PagingAndSorting pageSort, Page page, boolean addIf, int nextPage, String rel, Collection<Link> links) {
        if (null != page && addIf) {
    ...
    @SuppressWarnings({"unchecked"})
    protected <V extends Serializable> V stringToSerializable(String s, Class<V> targetType) {
    ...
    @SuppressWarnings({"unchecked"})
    protected <V> V readIncoming(HttpInputMessage request, MediaType incomingMediaType, Class<V> targetType) throws IOException {
    ...
    //  protected Set<Link> extractLinkedProperties(String repoRel,
    //                                            Object entity,
    ...
    //  }
    //
    //  protected Map<String, Object> extractEmbeddedProperties(String repoRel,
    //                                                        Object entity,
    //                                                        EntityMetadata<AttributeMetadata> entityMetadata,
    //                                                        URI baseUri) {
    ...
    protected boolean shouldReturnLinks(String acceptHeader) {
    ...
    protected boolean returnBody(ServletServerHttpRequest request) {
    ...
    protected <E extends RepositoryEvent> void publishEvent(E event) {
        if (null != applicationContext) {
            applicationContext.publishEvent(event);
        }
    }

    protected boolean notPagingParam(String name) {
        return (!config.getPageParamName().equals(name) && !config.getLimitParamName().equals(name) && !config.getSortParamName().equals(name));
    }

    @SuppressWarnings({"unchecked"})
    protected Map throwableToMap(Throwable t) {
        Map m = new HashMap();
    ...
    protected String objectToMapKey(Object obj) {
    ...
    protected ResponseEntity<byte[]> notFoundResponse(ServletServerHttpRequest request) throws IOException {
        return negotiateResponse(request, HttpStatus.NOT_FOUND, new HttpHeaders(), null);
    }

    @SuppressWarnings({"unchecked"})
    protected ResponseEntity<byte[]> errorResponse(ServletServerHttpRequest request, HttpStatus status, Throwable t) throws IOException {
        Object body = null;
    ...
    @SuppressWarnings({"unchecked"})
    protected ResponseEntity<byte[]> negotiateResponse(final ServletServerHttpRequest request, final HttpStatus status, final HttpHeaders headers, final Object resource) throws IOException {
    ...
    @SuppressWarnings({"unchecked"})
    protected HttpMessageConverter findWriteConverter(Class<?> type, MediaType mediaType) {
        for (HttpMessageConverter conv : config.getCustomConverters()) {
    ...
    protected ResponseEntity<byte[]> maybeWrapJsonp(HttpStatus status, String jsonpParam, String jsonpOnErrParam, HttpHeaders headers, byte[] body) {
