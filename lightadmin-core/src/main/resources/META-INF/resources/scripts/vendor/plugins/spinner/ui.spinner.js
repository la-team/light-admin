				...
				// in tab order
					...
					( // filter node types that participate in the tab order
						...
						// anchor tag
							...
							(/input|select|textarea|button/.test( nodeName ) && 'hidden' != a.type && !a.disabled)
							) &&
	...
	$.fn.extend( {
					 ...
					 ariaState: function ( state, value ) {
						 return (value !== undefined
							 ...
							 (isFF2 ? el.setAttributeNS( "http://www.w3.org/2005/07/aaa", "aaa:" + state, value ) : $( el ).attr( "aria-" + state, value ));
						 } )
...
(function ( $ ) {
	...
	$.widget( 'ui.spinner', {
		_init: function () {
			...
			var self = this;
			this.element.addClass( 'ui-spinner-box' ).attr( 'autocomplete', 'off' ); // switch off autocomplete in opera

			if ( isNaN( this._getValue() ) && this.options.allowNull ) {
				this.element.val( '' );
			} else {
				this._setValue( isNaN( this._getValue() ) ? this.options.start : this._getValue() );
			}

			this.element.wrap( '<div>' ).parent().addClass( 'ui-spinner' ).append( '<button class="ui-spinner-up" type="button"></button>' ).find( '.ui-spinner-up' ).bind( 'mousedown',function ( e ) {
			...
			if ( this._items > 1 ) {
				...
				this.element.addClass( 'ui-spinner-list' ).height( height ).children().addClass( 'ui-spinner-listitem' ).height( height ).css( 'overflow', 'hidden' ).end().parent().height( height ).end();
		...
		_cleanUp: function () {
			if ( (isNaN( this._getValue() ) || this._getValue() === '') && this.options.allowNull ) {
				this.element.val( '' );
			} else {
		...
		_keydown: function ( e ) {
			...
			return (e.keyCode == KEYS.TAB || e.keyCode == KEYS.BACKSPACE || e.keyCode == KEYS.LEFT || e.keyCode == KEYS.RIGHT || e.keyCode == KEYS.PERIOD || e.keyCode == KEYS.NUMPAD_DECIMAL || e.keyCode == KEYS.NUMPAD_SUBTRACT || (e.keyCode >= 96 && e.keyCode <= 105) || // add support for numeric keypad 0-9
		...
		_setValue: function ( newVal ) {
			...
			this.element.val( this.options.currency ? $.ui.spinner.format.currency( newVal, this.options.currency ) : $.ui.spinner.format.number( newVal, this._decimals ) );
		},
		_animate: function ( d ) {
		...
		_addItem: function ( obj, fmt ) {
			if ( !this.element.is( 'input' ) ) {
				...
				if ( typeof obj == 'object' ) {
					...
					html = format.replace( /%(\(([^)]+)\))?/g, (function ( data ) {
		...
		enable: function () {
			this.element.removeAttr( 'disabled' ).siblings().removeAttr( 'disabled' ).parent().removeClass( 'ui-spinner-disabled' );
			this.disabled = false;
		},
		disable: function () {
			this.element.attr( 'disabled', true ).siblings().attr( 'disabled', true ).parent().addClass( 'ui-spinner-disabled' );
			this.disabled = true;
		}
	} );
	...
	$.extend( $.ui.spinner, {
		...
		format: {
			...
			number: function ( num, dec ) {
				return isNaN( num ) ? 0 : parseFloat( num, 10 ).toFixed( dec );
			}
		}
	} );
