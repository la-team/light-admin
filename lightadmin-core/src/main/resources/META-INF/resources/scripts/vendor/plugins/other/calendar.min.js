...
(function ( m, oa ) {
	...
	function Zb( a, b ) {
		function e() {
			q = b.theme ? "ui" : "fc";
			if ( b.header ) {
				return Q = m( "<div class='fc-header' style='width:100%'/>" ).append( m( "<div>" ).append( f( "left" ) ).append( f( "center" ) ).append( f( "right" ) ) )
			}
		}
		...
		function f( u ) {
			...
			(u = b.header[u]) && m.each( u.split( " " ), function ( na ) {
				...
				m.each( this.split( "," ), function ( ra, sa ) {
					if ( sa == "title" ) {
						fa.append( "<span class='fc-header-title'><h5>&nbsp;</h5></span>" );
						...
						if ( ha ) {
							...
							var da = jb( b.buttonText, sa ), ma = m( "<span class='fc-button fc-button-" + sa + " " + q + "-state-default'><span class='fc-button-inner'><span class='fc-button-content'>" + (ra ? "<span class='fc-icon-wrap'><span class='ui-icon ui-icon-" + ra + "'/></span>" : da) + "</span></span></span>" );
							if ( ma ) {
								ma.click(function () {
	...
	function sb( a, b, e ) {
		...
		function l( w ) {
			...
			E.each( function ( wa, Ga ) {
				ea = m( Ga );
				aa = ca( wa );
				aa.getMonth() == R ? ea.removeClass( "fc-other-month" ) : ea.addClass( "fc-other-month" );
	...
	function pc() {
		...
		function e( U ) {
			var ca = da(), ka = ma(), qa = N( g.visStart );
			ka = ba( N( qa ), ka );
	...
	function Rb( a, b, e ) {
		...
		function g() {
			...
			for ( A = 0; A < Ba; A++ ) {
				x += "<td class='fc- fc-col" + A + " " + O + "'><div><div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></div></td>";
			}
		...
		function L( h, O, x ) {
			if ( x ) {
				i( "allDaySlot" ) && na( h, ba( N( O ), 1 ), true );
			} else {
				c( h, O )
			}
		}
	...
	function Mb() {
		...
		function d( Q ) {
			var q = f.cellDate, u = f.cellIsAllDay, fa = f.getHoverListener(), na = f.reportDayClick;
